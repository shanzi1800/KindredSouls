// VERCEL_REDEPLOY_TRIGGER_1782473985
...
function getZodiac(month, day) {
  for (const range of ZODIAC_DATES) {
    if (month === range.month && day <= range.day) console.error("DEBUG getZodiac("+month+","+day+") -> "+range.sign); return range.sign;
  }
  return "\u6469\u7FAF\u5EA7";
}
...
function getWealthTarot(birthDate, lang = "zh") {
  ...
  // Wealth-focused meaning — all 6 languages
  const wealthMeaning = {
    en: getWealthTarotMeaning(card.id, reversed, "en"),
    zh: getWealthTarotMeaning(card.id, reversed, "zh"),
    es: getWealthTarotMeaning(card.id, reversed, "es"),
    fr: getWealthTarotMeaning(card.id, reversed, "fr"),
    th: getWealthTarotMeaning(card.id, reversed, "th"),
    vi: getWealthTarotMeaning(card.id, reversed, "vi"),
  };
  return {
    id: card.id,
    name: card.name[L] || card.name.en,
    meaning: wealthMeaning[L] || wealthMeaning.en,
    emoji: card.emoji,
...
// Wealth tarot meanings — all 6 languages (unified)
function getWealthTarotMeaning(cardId, reversed, lang) {
  const D = {
    en: {
      0: ['Reckless spending ahead — pause before impulse investments.', 'New financial adventure begins — take calculated risks.'],
      1: ["Wasted potential — your skills aren't converting to income.", 'Manifest wealth now — your talents are ready to monetize.'],
      2: ['Intuition blocked — financial gut feeling is wrong today.', 'Trust financial intuition — hidden opportunities await.'],
      3: ['Financial neglect — stop ignoring money management.', 'Abundance flows — wealth grows with patient nurturing.'],
      4: ['Financial control issues — debts or overspending trap.', 'Build wealth structure — solid financial foundation established.'],
      5: ['Financial dogma blinds you — question money beliefs.', 'Wealth aligns with values — ethical money path clear.'],
      6: ['Financial choice paralysis — pick one investment path.', 'Wealth choice point — follow your financial heart.'],
      7: ['Financial direction scattered — focus or lose momentum.', 'Wealth chariot moves — determined financial action wins.'],
      8: ['Financial weakness — build money confidence now.', 'Inner financial strength — gentle wealth power awakens.'],
      9: ['Financial isolation — seek wealth mentor or advisor.', 'Wealth wisdom within — solitude brings money insights.'],
      10: ['Financial luck stagnant — force change now.', 'Wealth cycle turns — fortune favors bold money moves.'],
      11: ['Financial imbalance — audit money flow now.', 'Wealth justice — money karma balances perfectly.'],
      12: ['Financial obsession — let go of money fixation.', 'Wealth perspective shift — new money vision needed.'],
      13: ['Financial death — old money patterns must end.', 'Wealth transformation — old financial you dies, new emerges.'],
      14: ['Financial extremes — find middle money path.', 'Wealth balance — moderate money approach wins.'],
      15: ['Financial freedom begins — break money chains now.', 'Wealth shadow work — face money demons to win.'],
      16: ['Financial tower falls — rebuild wealth smarter.', 'Wealth breakthrough — sudden money shift incoming.'],
      17: ['Financial hope lost — keep faith in money path.', 'Wealth star guides — hope returns to money journey.'],
      18: ['Financial illusion — see money truth clearly now.', 'Wealth intuition peaks — lunar money magic works.'],
      19: ['Financial sunshine blocked — wealth still growing.', 'Wealth sun shines — financial success bright ahead.'],
      20: ['Financial awakening delayed — listen to money calling.', 'Wealth calling heard — financial rebirth approaches.'],
      21: ['Financial completion denied — finish money business.', 'Wealth cycle complete — financial world transforms.'],
    },
    zh: {
      0: ['冲动消费警报——在做风险投资前先停一停。', '新的财务冒险开启——谨慎计算后可以出手。'],
      1: ['潜力被浪费——你的技能没有转化为收入。', '财富显化时机已到——你的才华可以变现了。'],
      2: ['财务直觉受阻——今天的投资直觉不准。', '信任财务直觉——隐藏的机会在等你。'],
      3: ['财务被忽视——别再忽视理财了。', '财富正在流动——耐心培育下金钱会增长。'],
      4: ['财务控制出问题——债务或超支陷阱。', '建立财富结构——稳固的财务基础已建立。'],
      5: ['财务教条蒙蔽你——质疑你的金钱观。', '财富与价值观对齐——符合伦理的搞钱路清晰。'],
      6: ['财务选择瘫痪——选定一条投资路吧。', '财富选择关口——跟随你的财务直觉走。'],
      7: ['财务方向分散——不专注就会失去动力。', '财富战车前进——坚定的财务行动致胜。'],
      8: ['财务软弱——现在建立金钱自信。', '内在财务力量——温和的财富力量正在觉醒。'],
      9: ['财务孤立——寻找财富导师或顾问。', '财富智慧在内心——独处带来金钱洞见。'],
      10: ['财运停滞——现在就强制改变。', '财富周期转动——运气眷顾大胆的财务行动。'],
      11: ['财务失衡——现在审计资金流向。', '财富因果平衡——金钱业力完美清算。'],
      12: ['财务执念——放下对金钱的痴迷。', '财富视角转变——新的金钱视野出现。'],
      13: ['抗拒财务死亡——旧的金钱模式必须终结。', '财富蜕变——旧的财务你死亡，新的诞生。'],
      14: ['财务走极端——找到中庸之道。', '财富平衡——适度的金钱策略致胜。'],
      15: ['财务自由开始——现在打破金钱枷锁。', '财富阴影功课——直面金钱心魔才能赢。'],
      16: ['财务崩塌在即——更聪明地重建财富。', '财富突破——突然的金钱转折正在到来。'],
      17: ['财务希望失落——对金钱之路保持信心。', '财富之星指引——希望回到财富旅程中。'],
      18: ['财务幻象——现在看清金钱真相。', '财务直觉达到峰值——月相金钱魔法生效。'],
      19: ['财务阳光被挡——财富仍在增长。', '财富阳光照耀——财务成功在前方闪耀。'],
      20: ['财务觉醒延迟——倾听金钱的呼唤。', '财富觉醒被听见——财务重生正在靠近。'],
      21: ['财务未完成——现在结束金钱事务。', '财富周期完成——财务世界正在蜕变。'],
    },
    es: {
      0: ['Gasto impulsivo detectado — detente antes de invertir.', 'Nueva aventura financiera — toma riesgos calculados.'],
      1: ['Potencial desperdiciado — tus habilidades no generan ingresos.', 'Manifiesta riqueza ahora — tus talentos están listos para monetizarse.'],
      2: ['Intuición bloqueada — tuinstinto financiero está equivocado hoy.', 'Confía en tu intuición financiera — oportunidades ocultas te esperan.'],
      3: ['Descuido financiero — deja de ignorar la gestión del dinero.', 'La abundancia fluye — la riqueza crece con paciencia.'],
      4: ['Problemas de control financiero — deuda o sobregastos.', 'Construye estructura de riqueza — base financiera sólida.'],
      5: ['Dogma financiero te ciega — cuestiona tus creencias sobre el dinero.', 'Riqueza alineada con valores — camino ético claro.'],
      6: ['Parálisis de decisión financiera — elige un camino de inversión.', 'Punto de decisión financiera — sigue tu corazón.'],
      7: ['Dirección financiera dispersa — concéntrate o pierde impulso.', 'El carro de la riqueza avanza — la acción decisiva gana.'],
      8: ['Debilidad financiera — construye confianza en el dinero ahora.', 'Fortaleza financiera interior — poder gentil despierta.'],
      9: ['Aislamiento financiero — busca mentor de riqueza.', 'Sabiduría financiera interior — la soledad trae perspectivas.'],
      10: ['Suerte financiera estancada — fuerza el cambio ahora.', 'El ciclo de riqueza gira — la fortuna favorece movimientos audaces.'],
      11: ['Desequilibrio financiero — audita el flujo de dinero.', 'Justicia financiera — el karma del dinero se equilibra.'],
      12: ['Obsesión financiera — suelta la fijación con el dinero.', 'Cambio de perspectiva financiera — nueva visión del dinero.'],
      13: ['Muerte financiera — los patrones viejos deben terminar.', 'Transformación de riqueza — el viejo tú financiero muere.'],
      14: ['Extremos financieros — busca el camino medio.', 'Equilibrio financiero — la moderación gana.'],
      15: ['La libertad financiera comienza — rompe las cadenas del dinero.', 'Trabajo con la sombra financiera — enfrenta tus demonios.'],
      16: ['La torre financiera cae — reconstruye más inteligente.', 'Quiebre financiero — cambio repentino de dinero.'],
      17: ['Esperanza financiera perdida — mantén la fe.', 'La estrella financiera guía — la esperanza regresa.'],
      18: ['Ilusión financiera — ve la verdad del dinero.', 'Intuición financiera en su punto máximo — magia lunar.'],
      19: ['Sol financiero bloqueado — la riqueza aún crece.', 'El sol financiero brilla — éxito brillante adelante.'],
      20: ['Despertar financiero retrasado — escucha la llamada.', 'El llamado de la riqueza es escuchado — renacimiento.'],
      21: ['Finalización financiera negada — termina los asuntos.', 'Ciclo financiero completo — transformación total.'],
    },
    fr: {
      0: ["Dépenses impulsives — arrête-toi avant d'investir.", 'Nouvelle aventure financière — prends des risques calculés.'],
      1: ['Potentiel gaspillé — vos compétences ne génèrent pas de revenus.', 'Manifester la richesse maintenant — vos talents sont prêts.'],
      2: ["Intuition bloquée — votre instinct financier est faux aujourd'hui.", 'Faites confiance à votre intuition — des opportunités vous attendent.'],
      3: ["Négligence financière — arrêtez d'ignorer la gestion.", "L'abondance circule — la richesse grandit avec patience."],
      4: ['Problèmes de contrôle financier — dettes ou dépassements.', 'Construire la structure financière — base solide établie.'],
      5: ["Dogme financier — questionnez vos croyances sur l'argent.", 'Richesse alignée avec vos valeurs — chemin éthique clair.'],
      6: ["Paralysie des choix — choisissez une voie d'investissement.", 'Point de choix financier — suivez votre cœur.'],
      7: ["Direction dispersée — concentrez-vous ou perdez de l'élan.", "Le char de la richesse avance — l'action déterminée gagne."],
      8: ['Faiblesse financière — construisez votre confiance maintenant.', "Force financière intérieure — pouvoir doux s'éveille."],
      9: ['Isolement financier — cherchez un mentor.', 'Sagesse financière intérieure — la solitude apporte des perspectives.'],
      10: ['Chance financière stagnante — forcez le changement.', 'Le cycle de richesse tourne — la fortune favorise les audacieux.'],
      11: ['Déséquilibre financier — auditez vos flux.', "Justice financière — le karma de l'argent s'équilibre."],
      12: ['Obsession financière — lâchez prise.', 'Changement de perspective — nouvelle vision nécessaire.'],
      13: ['Mort financière — les anciens schémas doivent finir.', 'Transformation financière — le vieil vous meurt.'],
      14: ['Extrêmes financiers — trouvez le juste milieu.', 'Équilibre financier — la modération gagne.'],
      15: ['Liberté financière commence — brisez les chaînes.', "Travail sur l'ombre — affrontez vos démons."],
      16: ["Tour financière qui s'effondre — reconstruisez plus intelligent.", 'Percée financière — changement soudain.'],
      17: ['Espoir financier perdu — gardez la foi.', "L'étoile financière guide — l'espoir revient."],
      18: ['Illusion financière — voyez clairement.', 'Intuition financière à son apogée — magie lunaire.'],
      19: ['Soleil financier bloqué — la richesse grandit encore.', 'Le soleil financier brille — succès brillant devant.'],
      20: ["Éveil financier retardé — écoutez l'appel.", "L'appel de la richesse entendu — renaissance."],
      21: ['Clôture financière refusée — terminez vos affaires.', 'Cycle financier complet — transformation mondiale.'],
    },
    th: {
      0: ['ระวังการใช้จ่ายวูบวาบ — หยุดก่อนลงทุน', 'การเสี่ยงทางการเงินใหม่ — คำนวณความเสี่ยงก่อน'],
      1: ['ศักยภาพสูญเปล่า — ทักษะยังไม่แปลงเป็นรายได้', 'สร้างความมั่งคั่งตอนนี้ — พรสวรรค์พร้อมแล้ว'],
      2: ['สัญชาตญาณทางการเงินถูกบล็อก — วันนี้ไม่แม่น', 'ไว้ใจสัญชาตญาณ — โอกาสซ่อนอยู่รอคุณอยู่'],
      3: ['ละเลยการเงิน — หยุดเพิกเฉยเรื่องเงิน', 'เงินไหลมา — ความมั่งคั่งเติบโตด้วยความอดทน'],
      4: ['ปัญหาควบคุมการเงิน — หนี้หรือใช้จ่ายเกิน', 'สร้างโครงสร้างความมั่งคั่ง — ฐานะมั่นคงแล้ว'],
      5: ['ความเชื่อเรื่องเงินบดบัง — ตั้งคำถามกับความเชื่อเก่า', 'ความมั่งคั่งสอดคล้องค่านิยม — ทางที่ถูกต้องชัดเจน'],
      6: ['เลือกไม่ได้ — เลือกเส้นทางลงทุนสักทาง', 'จุดตัดสินใจเรื่องเงิน — ทำตามหัวใจ'],
      7: ['ทิศทางการเงินกระจัด — โฟกัสไม่ได้', 'รถม้าความมั่งคั่งวิ่ง — ความมุ่งมั่นชนะ'],
      8: ['ความอ่อนแอทางการเงิน — สร้างความเชื่อมั่นเลย', 'พลังการเงินภายใน — พลังอ่อนโยนตื่น'],
      9: ['โดดเดี่ยวทางการเงิน — หาที่ปรึกษา', 'ปัญญาความมั่งคั่งภายใน — ความสันโดษให้มุมมองใหม่'],
      10: ['โชคทางการเงินหยุดชะงัก — บังคับเปลี่ยนเลย', 'วงจรความมั่งคั่งหมุน — โชคสนับสนุนผู้กล้า'],
      11: ['ความไม่สมดุลทางการเงิน — ตรวจสอบกระแสเงิน', 'ความยุติธรรมทางการเงิน — กรรมเงินสมดุล'],
      12: ['หมกมุ่นเรื่องเงิน — ปล่อยวาง', 'มุมมองทางการเงินเปลี่ยน — ต้องการวิสัยทัศน์ใหม่'],
      13: ['ความตายทางการเงิน — รูปแบบเก่าต้องจบ', 'การเปลี่ยนแปลงความมั่งคั่ง — ตายแล้วเกิดใหม่'],
      14: ['ขั้วโลกทางการเงิน — หาทางสายกลาง', 'สมดุลความมั่งคั่ง — ทางเลือกปานกลางชนะ'],
      15: ['อิสรภาพทางการเงินเริ่มแล้ว — ทำลายโซ่ตรวน', 'ทำงานกับเงาทางการเงิน — เผชิญปีศาจเงิน'],
      16: ['หอคอยทางการเงินล่ม — สร้างใหม่ฉลาดกว่า', 'การทะลุทางการเงิน — เงินเปลี่ยนทิศฉับพลัน'],
      17: ['ความหวังทางการเงินสูญ — รักษาศรัทธา', 'ดาวนำทางความมั่งคั่ง — ความหวังกลับมา'],
      18: ['ภาพลวงตาทางการเงิน — เห็นความจริง', 'สัญชาตญาณทางการเงินสูงสุด — เวทมนตร์จันทรคติ'],
      19: ['ดวงอาทิตย์ทางการเงินบดบัง — ความมั่งคั่งยังโต', 'ดวงอาทิตย์ทางการเงินส่อง — ความสำเร็จรุ่งโรจน์'],
      20: ['การตื่นรู้ทางการเงินล่าช้า — ฟังเสียงเรียก', 'เสียงเรียกความมั่งคั่งดังแล้ว — การเกิดใหม่ใกล้'],
      21: ['การปิดบัญชีทางการเงินถูกปฏิเสธ — จบเรื่องเงินเถอะ', 'วงจรความมั่งคั่งสมบูรณ์ — โลกการเงินเปลี่ยน'],
    },
    vi: {
      0: ['Chi tiêu bộc phá — dừng lại trước khi đầu tư冲动。', 'Cuộc phiêu lưu tài chính mới — tính toán rủi ro trước。'],
      1: ['Tiềm năng lãng phí — kỹ năng chưa chuyển thành thu nhập。', 'Thể hiện của cải ngay bây giờ — tài năng sẵn sàng。'],
      2: ['Trực giác tài chính bị chặn — hôm nay linh cảm sai。', 'Tin vào trực giác tài chính — cơ hội ẩn đang chờ bạn。'],
      3: ['Bỏ bê tài chính — đừng lơ là việc quản lý tiền。', 'Cải tạo dồi dào — của cải lớn lên nhờ kiên nhẫn。'],
      4: ['Vấn đề kiểm soát tài chính — nợ hoặc chi tiêu quá tay。', 'Xây dựng cấu trúc tài sản — nền tảng vững chắc rồi。'],
      5: ['Giáo điều tài chính làm mù — hãy đặt câu hỏi về niềm tin tiền bạc。', 'Của cải phù hợp giá trị — con đường kiếm tiền đạo đức rõ ràng。'],
      6: ['Tê liệt quyết định tài chính — chọn một hướng đầu tư đi。', 'Điểm quyết định tài chính — theo trái tim tài chính của bạn。'],
      7: ['Hướng tài chính phân tán — tập trung hoặc mất đà。', 'Xe tài chính tiến — hành động kiên quyết thắng。'],
      8: ['Yếu đuối tài chính — xây sự tự tin tiền bạc ngay。', 'Sức mạnh tài chính bên trong — năng lượng dịu dàng thức tỉnh。'],
      9: ['Cô lập tài chính — tìm cố vấn tài sản。', 'Trí tuệ giàu có bên trong — một mình mang lại góc nhìn mới。'],
      10: ['May mắn tài chính đình trệ — buộc thay đổi ngay。', 'Chu kỳ giàu có quay — vận may ủng hộ người dám làm。'],
      11: ['Mất cân bằng tài chính — kiểm toán dòng tiền ngay。', 'Công lý tài chính — nghiệp tiền cân bằng hoàn hảo。'],
      12: ['Ám ảnh tài chính — buông bỏ sự bám víu tiền bạc。', 'Góc nhìn tài chính chuyển đổi — cần tầm nhìn mới về tiền。'],
      13: ['Cái chết tài chính — mô hình cũ phải kết thúc。', 'Chuyển đổi giàu có — người tài chính cũ chết, người mới ra đời。'],
      14: ['Cực đoan tài chính — tìm con đường giữa。', 'Cân bằng giàu có — chiến lược tiền bạc vừa phải thắng。'],
      15: ['Tự do tài chính bắt đầu — phá vỡ xiềng xích tiền。', 'Làm việc với bóng tối tài chính — đối mặt quỷ tiền bạc để thắng。'],
      16: ['Tháp tài chính sụp đổ — xây lại thông minh hơn。', 'Đột phá tài chính — chuyển đổi tiền bạc đột ngột。'],
      17: ['Hy vọng tài chính mất — giữ đức tin。', 'Ngôi sao dẫn đường giàu có — hy vọng quay lại。'],
      18: ['Ảo tưởng tài chính — thấy rõ sự thật tiền。', 'Trực giác tài chính đạt đỉnh — phép thuật trăng tròn。'],
      19: ['Ánh dương tài chính bị chắn — của cải vẫn đang lớn。', 'Ánh dương tài chính chiếu sáng — thành công rực rỡ phía trước。'],
      20: ['Giác ngộ tài chính trì hoãn — lắng nghe tiếng gọi。', 'Tiếng gọi giàu có được nghe — tái sinh đang đến gần。'],
      21: ['Hoàn thành tài chính bị từ chối — kết thúc chuyện tiền。', 'Chu kỳ giàu có hoàn tất — thế giới tài chính chuyển đổi。'],
    },
  };
  const meanings = D[lang] || D.en;
  return (meanings[cardId] || [])[reversed ? 0 : 1] || "Wealth insight unavailable.";
}
...
async function handler(req, res) {
  ...
  try {
    ...
    // ════════════════════════════════════════════════════════════════════════
    const isTestAccount = (body.free_access === 1) || (birthDate === '1990-06-15');
    ...
    console.error("DEBUG birthInfo=", JSON.stringify(birthInfo));
        ...
        if (userRes.ok) {
          ...
          if (!hasWealthAccess && paidPlans.all_pass_yearly === true) {
            ...
            if (!expiresAt || now < new Date(expiresAt)) {
              const monthlyResetAt = paidPlans.all_pass_wealth_monthly_reset_at
                ? new Date(paidPlans.all_pass_wealth_monthly_reset_at)
                : null;

              // 跨月自动归零
              if (!monthlyResetAt || now >= monthlyResetAt) {
                const nextReset = new Date(now.getUTCFullYear(), now.getUTCMonth() + 1, 1);
                // PATCH: reset monthly counter
                try {
                  await fetch(`${process.env.SUPABASE_URL}/rest/v1/user_profiles?user_id=eq.${encodeURIComponent(currentUserId)}`, {
                    ...
                    body: JSON.stringify({
                      paid_plans: {
                        ...paidPlans,
                        all_pass_wealth_monthly_used: 0,
                        all_pass_wealth_monthly_reset_at: nextReset.toISOString(),
                      }
                    }),
                  });
                } catch (resetErr) {
                  console.warn('[Wealth Oracle] Failed to reset monthly quota:', resetErr.message);
                }
              }

              const usedThisMonth = (paidPlans.all_pass_wealth_monthly_used || 0);
              if (usedThisMonth >= 5) {
        ...
        try {
          await fetch(`${process.env.SUPABASE_URL}/rest/v1/user_profiles?user_id=eq.${encodeURIComponent(currentUserId)}`, {
            ...
            body: JSON.stringify({
              paid_plans: {
                ...paidPlans,
                all_pass_wealth_monthly_used: (paidPlans.all_pass_wealth_monthly_used || 0) + 1,
              }
            }),
          });
        } catch (quotaErr) {
          console.error('[Wealth Oracle] Failed to record all_pass monthly quota:', quotaErr.message);
        }
      }
      // Save/update cache
      if (supabase) {
