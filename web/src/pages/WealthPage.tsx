...
const WealthPage: React.FC<WealthPageProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = () => {
    ...
    // 2026-08-26 军师补刀：只要选了 birthTime，就把 lat/lon/tz 带上。
    //   - 有城市 → 用城市精确坐标（上升星座精准）
    //   - 没城市 → 用时区默认坐标（太阳星座精准，上升兜底 Bangkok）
    if (birthTime) {
      ...
      if (birthCity) {
        ...
        // 兜底：没选城市时用 Asia/Bangkok 默认坐标，WealthReportPage 的 useState 默认值已对齐
        params.birthTz = 'Asia/Bangkok';
        params.birthLat = '13.75';
        params.birthLon = '100.5';
      }
    }
