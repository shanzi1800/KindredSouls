...
export function calcBaZi(p1: BirthInfo, p2: BirthInfo, lang: AlgLang = 'zh'): EngineResult {
  ...
  for (const w of ['木', '火', '土', '金', '水'] as const) {
    ...
    if (Math.abs(diff) >= 2 && (wx1[w] || 0) + (wx2[w] || 0) >= 3) {
      ...
      const wxNourish: Record<string,string> = { zh:'滋养', en:', nourishes ', es:', nutre a ', fr:', nourrit ', th:', ช่วยเลี้ยงดู', vi:', tương hỗ và điều hòa với ' };
  ...
  if (score >= 85) {
    const SUMMARY_85: Record<string,string> = {
      ...
      vi:`Nhật Chủ ${tg(sz1.dayMaster)} gặp ${tg(sz2.dayMaster)} — thiên can hòa hợp địa chi thuận duyên duyên phần trời`,
    };
  ...
  const detail = [
    ...
    `${BAZI_LABELS.detailYou[lang]} ${BAZI_LABELS.dayMaster[lang]} ${tg(sz1.dayMaster)}${lang==='zh' ? '（'+wx(TG_WUXING[sz1.dayMaster])+'）' : ''}，${BAZI_LABELS.detailTa[lang]} ${BAZI_LABELS.dayMaster[lang]} ${tg(sz2.dayMaster)}${lang==='zh' ? '（'+wx(TG_WUXING[sz2.dayMaster])+'）' : ''}。${rishiPhrase}`,
