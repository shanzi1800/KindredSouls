...
interface BaZiAnalysis {
  score: number;           // 0-100
  wuxingScore: number;     // 五行互补分
  rishiScore: number;      // 日主关系分
  hehunScore: number;      // 合婚加分（三合/六合/六冲扣分）
  details: string[];       // 分析要点
  p1Pillar: string;        // P1日柱
  p2Pillar: string;        // P2日柱
  p1Wuxing: string[];      // P1四柱五行分布
  p2Wuxing: string[];      // P2四柱五行分布
}
